...
contract('PrivateMarketplace', (accounts) => {
	...
	before('Deploy ERC-1155 and Marketplace contracts', async () => {
		// deploy NFT token
		this.IngredientNFT = await deployProxy(IngredientsNFT, [url], {initializer: 'initialize'});

		// deploy private marketplace
		this.privateMarketplace = await deployProxy(PrivateMarketplace, [this.IngredientNFT.address], {
		...
		await this.IngredientNFT.addExceptedAddress(this.privateMarketplace.address);
	...
	describe('initialize()', () => {
		...
		it('should initialize the NFT contract address correctly', async () => {
			...
			expect(this.IngredientNFT.address).to.be.eq(nftContractAddress);
		});
	});
	...
	describe('createAndSellNFT()', () => {
		...
		it('should generate sale id and nft id correctly', async () => {
			currentNftId = await this.IngredientNFT.getCurrentNftId();
			...
			expect(currentNftId).to.bignumber.be.eq(new BN('3'));
	...
	describe('canceleSale()', () => {
		...
		before('cancel sale', async () => {
			...
			privateMarketNFTBalBefore = await this.IngredientNFT.balanceOf(
		...
		it('should revert if non-seller tries to cancel the sale', async () => {
			...
			it('should delete sale details correctly', async () => {
				...
				privateMarketNFTBalAfter = await this.IngredientNFT.balanceOf(
					this.privateMarketplace.address,
					currentNftId
				);
		...
		it('should not cancel sale if some of the copies of NFT are sold in sale', async () => {
			...
			// buy nft from sale
			await this.privateMarketplace.buyNFT(currentSaleId, {from: user2});
	...
	describe('cancelAuction()', () => {
		...
		it('should not cancel auction with non-zero bids', async () => {
			...
			// place bid
			await this.privateMarketplace.placeBid(currentAuctionId, ether('4'), {from: user3});
	...
	describe('buyNFT()', () => {
		...
		before('buy nft from sale', async () => {
			...
			// transfer user1 current ingredients to another user
			await this.IngredientNFT.safeTransferFrom(
				user1,
				accounts[0],
				currentIngredientId,
				1,
		...
		it('should reflect nft in user wallet correctly', async () => {
			...
			const user1NFTBal = await this.IngredientNFT.balanceOf(user1, currentNftId);
	...
	describe('resolveAuction()', () => {
		...
		before('resolve auction', async () => {
			...
			user1NFTBalanceBefore = await this.IngredientNFT.balanceOf(user1, currentNftId);
			contractNFTBalanceBefore = await this.IngredientNFT.balanceOf(
				this.privateMarketplace.address,
				currentNftId
			);
		...
		it('should resolve the auction and update the auction status to close', async () => {
			...
			const user1NFTBalanceAfter = await this.IngredientNFT.balanceOf(user1, currentNftId);
			const contractNFTBalanceAfter = await this.IngredientNFT.balanceOf(
				this.privateMarketplace.address,
				currentNftId
			);
			...
			expect(contractNFTBalanceBefore).to.bignumber.be.eq(new BN('33'));
			expect(user1NFTBalanceAfter).to.bignumber.be.eq(new BN('1'));
			expect(contractNFTBalanceAfter).to.bignumber.be.eq(new BN('32'));
	...
	describe('getAuctionWinningBid()', () => {
		...
		it('should revert when anyone tries to get winning bid with invalid auction id', async () => {
			await expectRevert(
				this.privateMarketplace.getAuctionWinningBid(18),
				'Market: INVALID_AUCTION_ID'
			);
		});
	});
