 ...
 */
	...
	modifier onlyMinter() {
		require(hasRole(MINTER_ROLE, _msgSender()), 'ERC1155NFT: ONLY_MINTER_CAN_CALL');
		_;
	}

	modifier onlyValidNftId(uint256 _nftId) {
		require(_nftId > 0 && _nftId <= tokenCounter.current(), 'ERC1155: INVALID_NFT_ID');
		_;
	}
 ...
 */
	...
	function removeExceptedAddress(address _account) external virtual onlyAdmin {
		require(_account != address(0), 'ERC1155NFT: CANNOT_REMOVE_ZERO_ADDRESS');
	...
	function removeExceptedFromAddress(address _account) external virtual onlyAdmin {
		...
		require(exceptedFromAddressesLength > 0, 'ERC1155NFT: CANNOT_REMOVE_FROM_EMPTY_LIST');

		if (exceptedFromAddresses[exceptedFromAddressesLength - 1] == _account) {
			// remove excepted address
			exceptedFromAddresses.pop();
			return;
		}

		(bool isUserExists, uint256 userIndex) = isExceptedFromAddress(_account);

		require(isUserExists, 'ERC1155NFT: CANNOT_FIND_USER');

		// move excepted address to last
		if (exceptedFromAddressesLength > 1) {
			address temp = exceptedFromAddresses[exceptedFromAddressesLength - 1];
